...
else
{
    ...
    else
    {
        ...
        if ErrorLevel
        {
            ...
            {   
                UninstallerPath = %szDefaultDir%\Uninstall.exe /S ; 'Au_.exe' is child process
                ...
                if bContinue
                {
                    ...
                    else
                    {
                        ...
                        Process, WaitClose, explorer.exe, 4
                        if ErrorLevel ; The PID still exists
...
if bContinue
{
    ...
    else
    {
        ...
        WinWaitActive, 7-Zip 9.20 Setup, Choose Install Location, 3
        if ErrorLevel
        ...
        else
        {
            ...
            {
                WinWaitClose, 7-Zip 9.20 Setup, Choose Install Location, 3
                if ErrorLevel
                    TestsFailed("'7-Zip 9.20 Setup (Choose Install Location)' window failed to close despite 'Install' button being clicked.")
                else
                    TestsOK("'7-Zip 9.20 Setup (Choose Install Location)' window appeared, 'Install' button clicked and window closed.")
            }
        }
    }
}
...
if bContinue
{
    WinWait, 7-Zip 9.20 Setup, Installing, 3
    if ErrorLevel
...
if bContinue
{
    WinWait, 7-Zip 9.20 Setup, Completing, 3
    if ErrorLevel
        TestsFailed("'7-Zip 9.20 Setup (Completing)' window does not exist.")
    else
    {
        WinActivate ; 7-Zip 9.20 Setup, Completing
        WinWaitActive, 7-Zip 9.20 Setup, Completing, 3
        if ErrorLevel
            TestsFailed("Unable to activate existing '7-Zip 9.20 Setup (Completing)' window.")
        else
        {
            ...
            if bVisible = 1 ; Control is visible
            {
                ...
                else
                {
                    ...
                    {
                        WinWaitClose, 7-Zip 9.20 Setup, Completing, 3
                        if ErrorLevel
            ...
            else
            {
                ...
                else
                {
                    WinWaitClose, 7-Zip 9.20 Setup, Completing, 3
                    if ErrorLevel
                        TestsFailed("'7-Zip 9.20 Setup (Completing)' window failed to close bespite 'Finish' button being clicked.")
                    else
