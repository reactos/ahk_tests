...
if bContinue
{
    ...
    if not ErrorLevel
    {
        ...
        else
        {
            TestsFailed()
            WinGetTitle, title, A
            OutputDebug, %TestName%:%A_LineNumber%: Test failed: Unable to change 'Edit1' control text to '%A_ProgramFiles%\OllyDbg'. Active window caption: '%title%'.`n
        }
    }
    else
    {
...
if bContinue
{
    ...
    {
        TestsOK()
        OutputDebug, OK: %TestName%:%A_LineNumber%: The application has been installed, because '%ProgramExe%' was found.`n
    }
    else
    {
        TestsFailed()
        OutputDebug, %TestName%:%A_LineNumber%: Test failed: Something went wrong, can't find '%ProgramExe%'.`n
    }
}
